...
class CUSTdataset(Dataset):
    ...
    def __getitem__(self, index):
        # we only use the persona of the person who respond (in fact: yourself)
        your_persona = self.data[index]["your_persona"]
        history = self.data[index]["history"]
        responses = self.data[index]["responses"]
        label = self.data[index]["label"]
        
        return self.process(your_persona, history, responses, label)

    
    def process(self, your_persona, history, responses, label):
        bos, eos, yourself, partner = self.tokenizer.convert_tokens_to_ids(
            SPECIAL_TOKENS
        )
        sequence = history 
        sequence = [
        ...
        sequence_p = [[bos]] + your_persona + [[eos]]  
        ...
        instance["true_response_label"] = label
    ...
    def use_in_batch_negative_samples(self, batch):
        ...
        for i in range(len(batch)):
            ...
            instance["input_ids"] = torch.cat([instance["input_ids"][:start], golden_responses_cat], dim=0)
            instance["input_ids_mask"] = torch.cat([instance["input_ids_mask"][:start], golden_responses_mask_cat], dim=0)   
            instance["labels"] = torch.cat([instance["labels"][:start], labels_cat], dim=0)            
            instance["attention_mask"] = [1] * len(instance["input_ids"])      
